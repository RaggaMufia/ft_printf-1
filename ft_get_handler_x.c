...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/11 11:21:43 by tmashigo          #+#    #+#             */
/*   Updated: 2018/08/11 11:21:48 by tmashigo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			ft_print_x(uintmax_t digit, t_flist *lst, char *base)
{
	...
	if (digit >= 16)
	{
		...
		len += ft_get_print_char(base[digit % 16]);
	}
	else
		len += ft_get_print_char(base[digit % 16]);
	return (len);
...
static short		ft_hend_x_digit(t_flist *lst, uintmax_t digit)
{
	...
	if (lst->flags[0] == '0' && lst->flags[1] == '#' && digit != 0)
		len += (lst->spec == 'X') ? (ft_get_print_str("0X", -1)) :
		(ft_get_print_str("0x", -1));
