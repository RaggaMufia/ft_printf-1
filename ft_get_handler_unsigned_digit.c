...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_get_handler_unsigned_digit.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tmashigo <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/11 11:21:25 by tmashigo          #+#    #+#             */
/*   Updated: 2018/08/11 11:21:29 by tmashigo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char				ft_print_uns_digit(uintmax_t digit, t_flist *lst)
{
	...
	if (digit >= 10)
	{
		...
		len += ft_get_print_char((digit % 10) + '0');
	}
	else
		len += ft_get_print_char(digit + '0');
	return (len);
}
...
static char				ft_hend_uns_digit(t_flist *lst, uintmax_t digit)
{
	...
	while (lst->precision != -1 && lst->precision > 0)
		(len += ft_get_print_char('0')) && lst->precision--;
